...
class M_pemeriksaan extends CI_Model {
    ...
    public function get_pemeriksaan_all($jenis, $limit, $offset) {
        $sql = "SELECT a.*, b.* FROM tbl_kendaraan a join tbl_pemeriksaan b "
                . " ON b.id_kendaraan = a.no_uji ";

        if ($jenis == 'trayek') {
            $sql .= "WHERE a.kp_ijin_trayek!='' ORDER BY b.id_pemeriksaan DESC LIMIT $limit OFFSET $offset";
        } else {
            $sql .= "WHERE a.kp_ijin_operasi!='' ORDER BY b.id_pemeriksaan DESC LIMIT $limit OFFSET $offset";
        }
    ...
    public function get_checklist_pemeriksaan($jenis, $limit, $offset) {
        ...
        if ($jenis == 'trayek') {
            $sql = "SELECT a.*, b.*, b.id_pemeriksaan as id_tbl_pemeriksaan, c.*, d.* FROM tbl_kendaraan a join tbl_pemeriksaan b "
                    ...
                    . " WHERE a.kp_ijin_trayek!='' ORDER BY b.id_pemeriksaan DESC LIMIT $limit OFFSET $offset";
        } else {
            $sql = "SELECT a.*, b.*, b.id_pemeriksaan as id_tbl_pemeriksaan,  d.* FROM tbl_kendaraan a join tbl_pemeriksaan b "
                    . " ON b.id_kendaraan = a.no_uji "
                    . " LEFT JOIN tbl_checklist_kendaraan d ON b.id_pemeriksaan = d.id_pemeriksaan"
                    . " WHERE a.kp_ijin_operasi!='' ORDER BY b.id_pemeriksaan DESC LIMIT $limit OFFSET $offset";
        }

//        print_r($sql);
//        $sql = "SELECT a.*, b.*, b.id_pemeriksaan as id_tbl_pemeriksaan, c.*, d.* FROM tbl_kendaraan a JOIN tbl_pemeriksaan b ON a.no_uji = b.id_kendaraan "
//                . " JOIN tbl_trayek c ON a.id_trayek = c.id_trayek "
//                . " LEFT JOIN tbl_checklist_kendaraan d ON b.id_pemeriksaan = d.id_pemeriksaan"
//                . " WHERE a.kp_ijin_trayek != '' LIMIT $limit OFFSET $offset";
        $query = $this->db->query($sql);
        if ($query->num_rows() > 0) {
            $result = $query->result();
            $query->free_result();
    ...
    public function get_total_pemeriksaan($jenis) {
    ...
    public function get_total_hasil_pemeriksaan($jenis) {
        ...
        if ($jenis != null) {
            if ($jenis == 'trayek') {
                $sql .= " WHERE c.kp_ijin_trayek!=''";
            } else {
                $sql .= " WHERE c.kp_ijin_operasi!=''";
            }
        }
    ...
    public function get_total_checklist_pemeriksaan($jenis) {
//        $sql = "SELECT COUNT(a.*) as total FROM tbl_kendaraan a join tbl_pemeriksaan b "
//                . " ON b.id_kendaraan = a.no_uji "
//                . " JOIN tbl_trayek c ON a.id_trayek = c.id_trayek "
