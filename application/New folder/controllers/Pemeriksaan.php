...
class Pemeriksaan extends CI_Controller {
    ...
    public function cari_kendaraan_by_tanggal() {
        ...
        $total_row = $this->m_pemeriksaan->get_total_pemeriksaan($this->session->userdata('path_session'), $this->session->userdata('tanggal_session'));
        $per_page = 10;
        ...
        $a['pagi'] = _page($total_row, $per_page, 4, site_url('pemeriksaan/cari_kendaraan_by_tanggal_pagi/p'));
        $a['path'] = $this->session->userdata('path_session');
        $a['sub_title'] = 'Angkutan Barang';
        $a['data'] = $this->m_pemeriksaan->get_pemeriksaan_all($this->session->userdata('path_session'), $akhir, $awal, $this->session->userdata('tanggal_session'));
        $a['page'] = "pemeriksaan/list";
...
//            $SQL.= " A.kp_ijin_operasi != '' ";
    ...
    public function act_delete_trayek($id_pemeriksaan) {
        ...
        if ($this->m_pemeriksaan->delete($id_pemeriksaan)) {
            $this->m_pemeriksaan->delete_kwitansi($id_kendaraan);
